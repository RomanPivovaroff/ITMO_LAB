...
CREATE TABLE Human (
 ...
 age INT NOT NULL,
 CHECK (name SIMILAR TO '[A-Za-zА-Яа-я]+')
);
CREATE TABLE World (
 id SERIAL PRIMARY KEY,
 name TEXT,
...
INSERT INTO World (name, known) VALUES
 ...
 ('Tunnel', 'other', 'normal', (SELECT id FROM World WHERE name = 'Japet')),
 ('House', 'other', 'well', (SELECT id FROM World WHERE name = 'Earth')),
 ('unknown plain', 'plain', 'normal', 2);
